...
#define PORTENTA_H7_SLOW_PWM_ISR_GENERIC_HPP
  ...
  #define PORTENTA_H7_SLOW_PWM_VERSION            "Portenta_H7_Slow_PWM v1.2.0"
  
  #define PORTENTA_H7_SLOW_PWM_VERSION_MAJOR      1
  #define PORTENTA_H7_SLOW_PWM_VERSION_MINOR      2
  #define PORTENTA_H7_SLOW_PWM_VERSION_PATCH      0

  #define PORTENTA_H7_SLOW_PWM_VERSION_INT        1002000  
#endif
...
#define MAX_NUMBER_CHANNELS        16
    ...
    //void setPWM(const uint32_t& pin, const uint32_t& frequency, uint32_t dutycycle, timer_callback StartCallback = nullptr,
    //            timer_callback StopCallback = nullptr)
    {
      uint32_t period = 0;
    ...
    // period in us
    // Return the channelNum if OK, -1 if error
    int setPWM_Period(const uint32_t& pin, const uint32_t& period, const float& dutycycle, timer_callback StartCallback = nullptr,
                       timer_callback StopCallback = nullptr)  
    {     
      return setupPWMChannel(pin, period, dutycycle, (void *) StartCallback, (void *) StopCallback);      
    }    
